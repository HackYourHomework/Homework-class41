...
// ! Function under test
function sanitizeFruitBasket(arr, itemToBeDeleted) {
  return arr.filter((basketItem) => basketItem !== itemToBeDeleted);
}
...
describe('sanitizeFruitBasket', () => {
  test('should take two parameters', () => {
    const expected = 2;
    const returned = sanitizeFruitBasket.length;
  ...
  test('should return a new array that does not include the unwanted `lemon`', () => {
    ...
    const returned = sanitizeFruitBasket(fruitBasket, 'lemon');
    expect(returned).toEqual(expected);
  });
});
